...
export const Calendar: FC<CalendarProps> = ({ className, ...props }) => {
    const { selectedValue, selectionType, setValue, setEmptyValue } =
        useCalendar(props);
    const calendarMonth = generateCalendarMonth(props.year, props.month);
    const [firstWeek] = calendarMonth.weeks.length
        ? calendarMonth.weeks
        : [{ days: [] }];
    return (
            ...
            <div className="flex flex-1">
                {firstWeek.days.map(({ dayShortName }) => {
                    return (
                        <div
                            key={dayShortName}
                            className="flex-1 flex items-center justify-center w-10 h-10"
                        >
                ...
                return (
                    <div className="flex flex-1">
                        {days.map(({ date, iso }, index) => {
                            ...
                            const isWithinMonth =
                                dateObj.getMonth() + 1 === calendarMonth.month;
                            const isRangeType =
                                selectionType === "range" &&
                            ...
                            const isSingleAndSelected =
                                ...
                                isSameDay(
                                    dateObj,
                                    parseCalendarDate(selectedValue)
                                );
                            ...
                            if (isSelected && isRangeType) {
                                isSelectionLeftEnd = isSameDay(
                                    dateObj,
                                    parseCalendarDate(selectedValue.from)
                                );
                                isSelectionRightEnd = isSameDay(
                                    dateObj,
                                    parseCalendarDate(selectedValue.to)
                                );
                            }
                            const isSelectionEnd =
                                isSelectionLeftEnd || isSelectionRightEnd;
