...
import useCalendarDate, {
    CalendarDate,
} from "../../calendar/hooks/useCalendarDate";
...
export const DateSinglePicker: FC<
    Omit<DatePickerProps, "type" | "hidePresetRanges">
> = ({ onApply, onCancel, defaultValue, hideActions = false }) => {
    const form = useForm({
        mode: "onChange",
        defaultValues: {
            inputText:
                defaultValue !== undefined
                    ? toDatePickerInput(defaultValue)
                    : "",
        },
    });
                        ...
                        <div className="flex flex-row items-center gap-3">
                            <div className="flex-1 flex flex-col">
                                <DatePickerInput
                                    ...
                                    onChange={inputValue => {
                                        if (
                                            isValidDatePickerInput(inputValue)
                                        ) {
                                            calendarHeaderDate.setValue(
                                                toCalendarDate(inputValue)
                                            );
                                        }
                    ...
                    {!hideActions && (
                        <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-t border-gray-200">
                            ...
                            <Button
                                ...
                                className="flex-1"
                                onClick={() =>
                                    onApply?.(toCalendarDate(field.value))
                                }
                            />
                        </div>
                    )}
